...
contract C4Submission is Test {
    ...
    /// @dev Helper: EVM 6dp → L1 8dp wei (same as TokenMath.usdcEvmToL1Wei)
    function _evmToL1Wei(uint256 evm6dp) internal pure returns (uint64) {
        return uint64(evm6dp * 100);
    }

    function test_submissionValidity() public {
        // ═══════════════════════════════════════════════════════════
        //  F-50543: settle() ignores bridgeRetentionAmount
        //  Severity: Medium — DoS of keeperBridge()
        // ═══════════════════════════════════════════════════════════

        // ── Step 1: Governor sets bridge retention to 500k USDC ──
        vm.startPrank(admin);
        vault.setBridgeRetentionAmount(500_000e6);
        vm.stopPrank();

        // ── Step 2: User deposits 600k USDC ──
        _deposit(user1, 600_000e6);
        assertEq(usdc.balanceOf(address(vault)), 600_000e6);

        // ── Step 3: Mock L1 backing so Accountant sees positive surplus ──
        // We need distributableSurplus() > proposedYield.
        // surplus = totalBackingSigned() - usdm.totalSupply()
        // totalBackingSigned includes L1 spot USDC (0x801 precompile).
        // usdm.totalSupply() = 600k (from deposit)
        // Mock 800k L1 USDC → surplus = 800k - 600k = 200k
        // shortfall = 0 → distributableSurplus = 200k
        _mockVaultL1SpotUsdc(_evmToL1Wei(800_000e6)); // 800k USDC on L1

        // ── Step 4: Advance time past minSettlementInterval (20h) ──
        vm.warp(block.timestamp + 21 hours);

        // ── Step 5: Operator settles 200k yield ──
        // settle() sees available = 600k - 0 = 600k → allows 200k
        // But bridgeRetentionAmount is 500k, so only 100k should be available
        vm.prank(operator);
        vault.settle(200_000e6);

        // ── Step 6: Verify yield moved to YieldEscrow ──
        assertEq(usdc.balanceOf(address(vault)), 400_000e6);
        assertEq(usdc.balanceOf(address(yieldEscrow)), 200_000e6);

        // ── Step 7: Advance time past bridgeInterval (6h) ──
        vm.warp(block.timestamp + 7 hours);

        // ── Step 8: keeperBridge() reverts with "nothing to bridge" ──
        // netBridgeable() = 400k - 0 - 500k = -100k → 0
        vm.prank(operator);
        vm.expectRevert("nothing to bridge");
        vault.keeperBridge(MonetrixVault.BridgeTarget.Vault);

        // ── Step 9: Confirm netBridgeable() is 0 ──
        assertEq(vault.netBridgeable(), 0);

        // ── Bridge to L1 is completely DoS'd ──
        // The protocol cannot move funds to L1 for hedge/HLP/BLP operations
        // until new user deposits bring vault balance above 500k again.
    }
}
