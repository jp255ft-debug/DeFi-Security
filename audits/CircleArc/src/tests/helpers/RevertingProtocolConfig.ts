...
export const revertingProtocolConfigArtifact = hre.artifacts.readArtifactSync('RevertingProtocolConfig')

export class RevertingProtocolConfig {
  static deploy = async (wallet: WalletClient, client: PublicClient) => {
    const receipt = await wallet
      .deployContract({
        abi: revertingProtocolConfigArtifact.abi,
        bytecode: revertingProtocolConfigArtifact.bytecode,
        args: [],
        value: 0n,
      })
