...
# Remove stale IPC sockets left by a previous SIGKILL / docker kill.
# Without this, Reth fails to bind() on restart with EOPNOTSUPP.
rm -f /sockets/*.ipc
...
if [ -n "$ARC_LOG_FILE" ]; then
	...
	exec "/usr/local/bin/arc-node-execution" "$@" > /tmp/arc-log-pipe 2>&1
else
	exec "/usr/local/bin/arc-node-execution" "$@"
fi
